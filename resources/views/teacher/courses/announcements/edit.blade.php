...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="mt-4 text-2xl font-semibold tracking-tight text-neutral-900">{{ __('Edit announcement') }}</h1>
        <p class="mt-1 text-sm text-neutral-600">{{ $course->title }}</p>
    </div>

    <x-ui.card>
        <form method="POST" action="{{ route('teacher.courses.announcements.update', [$course, $announcement]) }}" class="space-y-6">
            @csrf
            @method('PATCH')
            ...
            <div class="flex gap-3">
                <x-primary-button>{{ __('Update announcement') }}</x-primary-button>
